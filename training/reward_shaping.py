...
class OnlineSepsisReward:
    ...
    3) Injects the generated action for the target role.
    4) Calls /reset then /step on the live environment.
    ...
    def _step(self, actions: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        r = requests.post(f"{self.env_url}/step", json={"actions": actions}, timeout=30)
        r.raise_for_status()
        return r.json()
    ...
    def __call__(self, completions: List[str], prompts: List[str], **kwargs: Any) -> List[float]:
        ...
        for i, (completion, prompt) in enumerate(zip(completions, prompts)):
            ...
            try:
                bundle = self._reset(self.seed + i)
